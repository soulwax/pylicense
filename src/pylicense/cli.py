...
def parse_args(args: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Manage license headers in your project files."
    )
    parser.add_argument(
        "-d",
        "--directory",
...
def process_verify_mode(project_root: Path, template: str) -> int:
    ...
    logging.info(
        ...
        files_with_license, total_files, percentage
    )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        if args.create_license_file:
            return process_create_license_file_mode(
                project_root, args.template, author, args.year
            )
